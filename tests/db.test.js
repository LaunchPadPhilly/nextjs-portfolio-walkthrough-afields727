...
vi.mock('@prisma/client', () => {
  ...
  return {
    PrismaClient: vi.fn(() => ({
      project: {
        findMany: vi.fn((args) => {
            if(args && args.where && args.where.title && args.where.title.contains) {
                return Promise.resolve(mockProjects.filter(p => p.title.includes(args.where.title.contains)));
            }
        ...
        create: vi.fn((args) => {
          if(!args.data.title || !args.data.description || !args.data.technologies) {
            return Promise.reject(new Error("Missing required fields"));
          }
          const newProject = { id: idCounter++, createdAt: new Date(), updatedAt: new Date(), imageUrl: null, projectUrl: null, githubUrl: null, ...args.data };
        ...
        update: vi.fn((args) => {
          ...
          mockProjects[index] = { ...mockProjects[index], ...args.data, updatedAt: new Date() };
        ...
        deleteMany: vi.fn((args) => {
            const titleFilter = args.where && args.where.title && args.where.title.contains;
            if (titleFilter) {
                const initialLength = mockProjects.length;
                const projectsToDelete = mockProjects.filter(p => p.title.includes(titleFilter));
                projectsToDelete.forEach(p => {
                    const index = mockProjects.findIndex(mp => mp.id === p.id);
                    mockProjects.splice(index, 1);
                })
                return Promise.resolve({ count: projectsToDelete.length });
            }
            return Promise.resolve({ count: 0 });
        }),
        createMany: vi.fn((args) => {
            const newProjects = args.data.map(p => ({ id: idCounter++, createdAt: new Date(), updatedAt: new Date(), imageUrl: null, projectUrl: null, githubUrl: null, ...p }));
            mockProjects.push(...newProjects);
            return Promise.resolve({ count: newProjects.length });
        })
      },
    }))
  };
